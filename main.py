...
def build_packet(msg_type: int, payload_dict=None) -> bytes:
    ...
    length = 3 + len(payload)  # size(2) + type(1) + payload (UInt16)
...
class TerrariaClient:
    def __init__(self, host: str, port: str, password: Optional[str] = None):
        self.host = host
        self.port = port
    ...
    def login(self):
        with socket.create_connection((self.host, self.port)) as s:
            ...
            send(s, 0x01, {"version": "Terraria279"})
...
if __name__ == "__main__":
    client = TerrariaClient(HOST, PORT)
    client.login()
