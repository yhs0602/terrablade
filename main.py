...
with socket.create_connection((HOST, PORT)) as s:
    ...
    send(
        s, 0x01, {"version": "Terraria37"}
    )  # 예시 버전 문자열 [oai_citation:0‡seancode.com](https://seancode.com/terrafirma/net.html)

    # 응답: $03 승인 or $25 비번요구 or $02 차단
    msg = recv_message(s)
    if msg.type == 0x25:  # Request Password
        send(s, 0x26, {"password": "mypassword"})  # $26 Login with Password
        msg = recv_message(
            s
        )  # 다시 $03 승인 or $02 실패 [oai_citation:1‡seancode.com](https://seancode.com/terrafirma/net.html)
    if msg.type == 0x02:
        raise SystemExit(f"banned or error: {msg.payload.error}")
    assert (
        msg.type == 0x03
    )  # Connection Approved, 이제 상태 Initializing(1) [oai_citation:2‡seancode.com](https://seancode.com/terrafirma/net.html)
    ...
    # $10 Life, $2A Mana, $32 Buffs (응답 기다리지 않고 전송) [oai_citation:3‡seancode.com](https://seancode.com/terrafirma/net.html)
    ...
    # 인벤토리 슬롯 0..72, $05 반복 전송 [oai_citation:4‡seancode.com](https://seancode.com/terrafirma/net.html)
    for inv in range(73):
        send(
            s,
            0x05,
    ...
    # $06 World Info 요청 [oai_citation:5‡seancode.com](https://seancode.com/terrafirma/net.html)
    send(s, 0x06)

    # 서버는 문제 있으면 $02로 킥. 정상이면 $07 응답 후 Initialized(2)로 승격 [oai_citation:6‡seancode.com](https://seancode.com/terrafirma/net.html)
    world_info = None
    while True:
        msg = recv_message(s)
        if msg.type == 0x02:
    ...
    # $08 초기 타일 데이터 요청. $07에서 받은 스폰 X,Y 사용 [oai_citation:7‡seancode.com](https://seancode.com/terrafirma/net.html)
    send(
        s,
        0x08,
    ...
    # 서버는 $09, 여러 개의 $0A, $0B, $15, $16, $17, $31, $39, $38 순으로 보냄 [oai_citation:8‡seancode.com](https://seancode.com/terrafirma/net.html)
    got_spawn = False
    while not got_spawn:
        msg = recv_message(s)
    ...
    # $0C Player Spawn 전송 → 상태 Playing(10) [oai_citation:9‡seancode.com](https://seancode.com/terrafirma/net.html)
    send(
        s,
        0x0C,
        {
